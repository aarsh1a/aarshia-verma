...
export default class Renderer {
    ...
    setComposition() {
        ...
        this.composition.final.camera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1)
    }
    ...
    resize() {
        ...
        // Render targets
        this.composition.space.setSize(this.config.width * 2, this.config.height * 2)
        this.composition.distortion.setSize(this.config.width * 0.5, this.config.height * 0.5)
    }
    ...
    update() {
        ...
        this.instance.render(this.composition.final.scene, this.composition.final.camera)
    ...
    destroy() {
        ...
        if (this.composition) {
            if (this.composition.space) this.composition.space.dispose()
            if (this.composition.distortion) this.composition.distortion.dispose()
        }
    }
}